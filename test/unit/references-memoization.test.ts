...
describe('references memoization', () => {
  ...
  it('returns CURSOR_EXPIRED when snapshot is evicted', async () => {
    const repoRoot = path.resolve(__dirname, '..', '..', '..');
    const tempDir = fs.mkdtempSync(path.join(repoRoot, 'tmp-references-expired-'));
    const tempFile = path.join(tempDir, 'file.txt');
    fs.writeFileSync(tempFile, 'const x = 1;', 'utf8');
    ...
    const disposable = vscode.languages.registerReferenceProvider(
      { scheme: 'file', language: 'plaintext' },
      {
        provideReferences: async () => locations,
      },
    );
  ...
  it('returns CURSOR_STALE when snapshot changes', async () => {
    const repoRoot = path.resolve(__dirname, '..', '..', '..');
    const tempDir = fs.mkdtempSync(path.join(repoRoot, 'tmp-references-stale-'));
    const tempFile = path.join(tempDir, 'file.txt');
    fs.writeFileSync(tempFile, 'const x = 1;', 'utf8');
  ...
  it('returns SNAPSHOT_TOO_LARGE when snapshot cannot be retained', async () => {
    const repoRoot = path.resolve(__dirname, '..', '..', '..');
    const tempDir = fs.mkdtempSync(path.join(repoRoot, 'tmp-references-large-'));
    const tempFile = path.join(tempDir, 'file.txt');
    fs.writeFileSync(tempFile, 'const x = 1;', 'utf8');
