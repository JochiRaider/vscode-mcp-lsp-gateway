...
function readAndValidateSettings(): { settings?: GatewaySettings; problems: string[] } {
  ...
  const additionalAllowedRoots = (cfg.get<readonly string[]>("additionalAllowedRoots", []) ?? []).map(
    String,
  );
  ...
  if (bindAddress !== "127.0.0.1") problems.push(`bindAddress must be "127.0.0.1" (got "${bindAddress}").`);

  // v1 invariant: single endpoint /mcp.
  if (endpointPath !== "/mcp") problems.push(`endpointPath must be "/mcp" (got "${endpointPath}").`);

  // Minimal sanity checks (package.json already constrains these, but we still fail closed).
  if (!Number.isInteger(port) || port < 1024 || port > 65535) problems.push(`port must be an integer in [1024, 65535] (got "${port}").`);

  if (!Number.isInteger(maxItemsPerPage) || maxItemsPerPage < 1 || maxItemsPerPage > 200) problems.push(`maxItemsPerPage must be an integer in [1, 200] (got "${maxItemsPerPage}").`);

  if (!Number.isInteger(maxResponseBytes) || maxResponseBytes < 16_384 || maxResponseBytes > 524_288) problems.push(`maxResponseBytes must be an integer in [16384, 524288] (got "${maxResponseBytes}").`);

  if (!Number.isInteger(requestTimeoutMs) || requestTimeoutMs < 250 || requestTimeoutMs > 2_000) problems.push(`requestTimeoutMs must be an integer in [250, 2000] (got "${requestTimeoutMs}").`);
...
async function setBearerTokensCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (!settings) {
    void vscode.window.showErrorMessage(`Cannot set tokens: invalid configuration. ${problems.join(" ")}`);
    return;
  }
  ...
  void vscode.window.showInformationMessage(`Stored ${tokens.length} bearer token(s) in SecretStorage.`);
}

async function clearBearerTokensCommand(context: vscode.ExtensionContext): Promise<void> {
  const { settings, problems } = readAndValidateSettings();
  if (!settings) {
    void vscode.window.showErrorMessage(`Cannot clear tokens: invalid configuration. ${problems.join(" ")}`);
    return;
  }
...
async function copyEndpointUrlCommand(): Promise<void> {
  const { settings, problems } = readAndValidateSettings();
  if (!settings) {
    void vscode.window.showErrorMessage(`Cannot copy endpoint URL: invalid configuration. ${problems.join(" ")}`);
    return;
  }
...
class ExtensionRuntime {
  ...
  public async ensureStartedIfEnabled(): Promise<void> {
    ...
    if (!settings) {
      await this.stopServer();
      this.output.appendLine(`[error] Invalid configuration; server will not start. ${problems.join(" ")}`);
      return;
    }
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mcpLspGateway.setBearerTokens", () => setBearerTokensCommand(context)),
    vscode.commands.registerCommand("mcpLspGateway.clearBearerTokens", () => clearBearerTokensCommand(context)),
    vscode.commands.registerCommand("mcpLspGateway.copyEndpointUrl", () => copyEndpointUrlCommand()),
  );
